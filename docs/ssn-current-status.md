...
pass. The production-shaped inactive lifecycle round then added exact
allowlisted non-fixture inactive applies, a Slurm-backed archive runner,
archive job metadata in user state/status, and microsecond user/state backups.
Live Quadro testing created fake `ssn-lifecycle-*` users only, left `adhil`
and `roshan` untouched, verified missing-allowlist failure before token
consumption, local-only archive job 85, hook-failure job 86, hook-success job
87, running job 92 cancellation, original UID/GID reactivation, wrong-plan
token rejection, installed-command CPU/GPU smoke jobs 97/98, and a final idle
node with an empty queue. A new Ansible root archive-association idempotence
bug was found and fixed; the node was manually resumed after the previous
failed drained apply had left it drained. The user/storage production bridge
then added all-managed quota dry-run reporting, protected-user validation for
`users.yml`, production-root retention cleanup with reviewed tokens,
allowlisted user scratch cleanup for cache/tmp paths, and a nondurable-storage
acknowledgment gate for install/apply. Live Quadro testing created only
`ssn-test-storage-*` users, verified all-managed quota reporting without
mutation, applied tiny quotas only to `ssn-test-storage-quota`, confirmed
over-quota writes fail on home/data/scratch, deleted only temporary SSN
retention artifacts under the allowed plans/users/fstab backup roots, skipped a
retention symlink, deleted only allowlisted fake-user scratch cache/tmp
candidates, rejected scratch wrong-plan and token-reuse attempts, left `adhil`
and `roshan` unlocked, and finished with CPU/GPU/scratch/no-requeue regression
jobs 99/100/101 and an idle node.
...
quota enablement on the disposable Quadro host. The user/storage bridge now
also has report-only all-managed quota drift visibility, tokenized deletion
for SSN-owned retention roots, exact-user tokenized scratch cache/tmp cleanup,
and an install/apply gate for nondurable data acknowledgments. Validate-only module/CUDA
...
produce a clean no-op dry-run after repair, and inactive lifecycle work now has
both fixture and fake non-fixture coverage: reviewed local-only archive tokens,
backup-hook-gated archive removal, Slurm-backed archive execution through the
archive service account/QoS, exact-user allowlists for non-fixture destructive
apply, and UID/GID reactivation checks.

The largest remaining gaps are real-user quota apply beyond report-only
visibility, production-wide login/GPU isolation beyond fixtures, multi-GPU
topology/NVML/CUDA ordering health gates, real external backup hook integration
beyond dummy hooks, production inactive rollout for real managed users,
permanent tombstone migration controls, managed CUDA/module update workflows,
CUDA toolkit live-module testing on a host with a toolkit installed, and
...
| Plan artifacts live under `/var/lib/slurm-single-node/plans`, mode `0750/0640`, retained 90 days. | Install and apply reports plus rendered artifacts are written under protected per-run plan dirs. Tokenized retention cleanup can now delete reviewed direct-child artifacts under SSN-owned production roots: plans, user backups, and fstab backups. Live Quadro deleted only temporary `ssn-test-storage-retention-*` artifacts from those roots, skipped a symlink, and rejected token reuse. | Implemented partially | `ssn/install.py`, `ssn/cli.py`, `ssn/safety.py`, live Quadro retention cleanup test | Decide whether to schedule production retention deletion or keep it as manual reviewed cleanup. |
...
| Risky operations require reviewed plan id/hash tokens. | Implemented for queued-jobs risk on install/apply, fixture-only scratch cleanup deletion, allowlisted user scratch cleanup, fixture inactive local-only archive apply, backup-hooked fixture inactive archive apply, exact-allowlisted fake non-fixture inactive apply, fixture CPU-only GPU recovery, production-root retention cleanup, and test-artifact retention deletion. Tokens are config/input/operation-hash-bound where available, expiring, stored hashed, and single-use. Live tests covered missing-allowlist refusal before token consumption, no-hook pre-consumption refusal, hook-backed inactive apply, token reuse rejection, wrong-plan token rejection, and operation-hash mismatch rejection. | Implemented partially | `ssn/ops.py`, `ssn/install.py`, `ssn/cli.py`, `ssn/storage.py`, `ssn/users.py`, `ssn/gpu.py`, `ssn/safety.py`, `bin/ssn-plan-token`, live Quadro token tests | Extend the same token pattern to real managed-user inactive rollout and real-user quota apply when approved. |
...
| Core helper commands exist. | All listed commands exist, plus installer, scratch cleanup, retention cleanup, storage quotas, scratch health, login isolation/status, GPU collector, GPU recovery commands, and the internal `ssn-archive-runner`. `ssn-archive-status` reports archive state, local-only status, backup status, hook path/rc, archive job id/state, service account, QoS, runner payload/result paths, errors, and next action. | Implemented correctly | `bin/`, `ssn/cli.py`, live Quadro archive/retention/quota/lifecycle tests | Harden status output for interrupted archive jobs and real backup-hook retries. |
...
| QoS is tier authority with one default account. | Implemented via `sacctmgr` account/QoS setup. The archive service account/QoS foundation is also present and root is associated for internal archive jobs. | Implemented correctly | `ansible/roles/ssn_slurm_config/tasks/main.yml`, live Quadro archive jobs | None for current account/QoS foundation. |
...
| Inactive reactivation must reuse original UID/GID. | Planning validates this case, and live Quadro reactivation without explicit IDs was rejected after tombstoning. Reactivation with original UID/GID recreated `ssn-test-inactive` and fake non-fixture `ssn-lifecycle-restore`, then restored `/data` ownership. | Implemented correctly | `ssn/users.py`, tests, live Quadro inactive/reactivation tests | Broaden restore workflow to real managed users only after production approval. |
| Permanent UID/GID tombstones. | Inactive apply records original UID/GID and reaches `archive_state: tombstoned` after account removal for fixture and fake non-fixture users. Reactivation consumes the original IDs. The quota round found that auto-created users could reuse tombstoned IDs; SSN now rejects explicit tombstone reuse, flags existing users occupying tombstoned IDs, and auto-allocates around tombstones. Production admin migration/clear controls remain incomplete. | Implemented partially | `ssn/users.py`, tests, live Quadro `ssn-test-inactive-prod`, `ssn-test-quota`, and `ssn-lifecycle-*` tests | Add admin migration/clear workflow and broader restore/conflict tests. |
| Back up `users.yml` and `users-state.yml` before writes. | CLI backs up both existing files before apply and reports 90-day retention candidates without deleting them. Backup names now include microseconds after live lifecycle testing exposed same-second backup collisions. | Implemented partially | `ssn/cli.py`, `ssn/users.py`, live Quadro sync/lifecycle apply | Add approved retention pruning if desired. |
...
| Admin-exempt users in profile/site config. | Profiles define admins; Ansible creates admin group memberships, and login/GPU isolation commands exclude configured admins in tested scopes. | Implemented partially | `profiles/*.yml`, `ssn_base` role, `ssn/login.py`, live Quadro isolation tests | Re-test admin exemptions when broad all-managed rollout is enabled. |
...
| Suspended lifecycle blocks login/Slurm and kills jobs. | Apply locks the Unix account, removes the default Slurm association, and runs `scancel`; live tests killed a pending fixture job and locked fake non-fixture `ssn-lifecycle-suspended`. PAM/login denial is basic account lock only. | Implemented differently | `ssn/users.py`, live Quadro suspended fixture and fake lifecycle user | Add complete PAM/login denial validation and decide whether association removal is the final Slurm-disable mechanism. |
| Inactive lifecycle archives, prunes, removes after backup. | Fixture and fake non-fixture inactive lifecycles are implemented behind reviewed tokens. Fake non-fixture apply requires exact `--allow-lifecycle-user`, refuses protected users, submits archive work through Slurm under the archive service account/QoS, records job metadata, and was live-tested for local-only archive, hook failure, hook success/removal, running-job cancellation, tombstone, and original UID/GID restore. Real backup/replication hooks are not integrated yet. | Implemented partially | `ssn/users.py`, `ssn/cli.py`, `bin/ssn-archive-runner`, live Quadro `ssn-lifecycle-*` tests | Add real external backup hooks and approve real managed-user rollout. |
...
| RAID0 `/data` persistent but not durable, external backup required. | Policy records nondurable data requirements, and install/apply now fail when a profile with `nondurable_ack_required` lacks `operations.storage_acknowledgements.nondurable_data: true`. The disposable Quadro profile sets that acknowledgment explicitly; generic production profiles remain unacknowledged until site review. | Implemented correctly | `policies/storage.yml`, `profiles/gpu-bisect-quadro-p620.yml`, `ssn/ops.py`, live Quadro drained install | Keep production profiles unacknowledged until backup/storage review is complete. |
...
| Quota-managed home/data/scratch and quota capability validation. | `ssn-storage-quotas plan/enable/status` can token-gate quota enablement, back up `/etc/fstab`, add `usrquota,grpquota`, remount ext4 mounts, run `quotacheck`, and enable quotas. Live Quadro enabled quotas on `/` for `/home`, plus `/data` and `/scratch`; install/apply gates now fail quota-required profiles when required user quotas are inactive. `ssn-sync-users --quota-report --quota-scope all_managed` now reports all active managed users, current quota evidence, and drift without mutation. Fixture quota apply supports tiny overrides and was live-tested for `ssn-test-quota` and then only `ssn-test-storage-quota`; real-user quota apply remains disabled. | Implemented partially | `ssn/storage.py`, `ssn/cli.py`, `ssn/ops.py`, `bin/ssn-storage-quotas`, live Quadro quota enable/enforcement and all-managed report tests | Add reviewed real-user quota apply only after production quota values are approved. |
...
| Scratch cleanup deletes eligible aged files, excluding job scratch. | Default cleanup remains report-only, and legacy fixture cleanup still supports top-level `/scratch/ssn-test-*` candidates. A production-shaped exact-user cleanup path now deletes only reviewed, tokenized candidates under `/scratch/$USER/cache` and `/scratch/$USER/tmp`, skips `/scratch/jobs`, symlinks, protected roots, non-allowlisted users, and users with active Slurm jobs. Live Quadro deleted only fake `ssn-test-storage-cleanup` cache/tmp candidates, rejected a wrong-plan token and token reuse, and left a symlink untouched. | Implemented partially | `ssn/storage.py`, `ssn-scratch-cleanup`, live Quadro cleanup token tests | Decide whether to enable scheduled production cleanup or keep exact-user manual cleanup only. |
...
| Decision / Requirement | Current Code State | Status Bucket | Evidence | Follow-up Needed |
|---|---|---|---|---|
| Kill jobs, lock data, prune, archive, backup hook, remove account after success. | Fixture and fake non-fixture paths are implemented under reviewed tokens. Live Quadro verified local-only fake user job 85, hook-failure job 86 with the account locked/present in `backup_failed`, hook-success job 87 with `backup_complete` and account removal, running job 92 cancellation before archive, `/data/$USER` lockout, tombstone, and reactivation. Real-user rollout remains disabled. | Implemented partially | `ssn/users.py`, `ssn/cli.py`, `bin/ssn-archive-runner`, live Quadro `ssn-lifecycle-*` tests | Add real external backup hooks and production approval before real users. |
| Archive job under service/admin Slurm identity and protected QoS. | Implemented for inactive archive apply. An archive Slurm account and `archive-admin` QoS are created by Ansible; `ssn-sync-users --apply` submits the internal archive runner as a monitored Slurm job using that service account/QoS and records job id/state/payload/result in user state. Live fake non-fixture archive jobs 85, 86, 87, 88, and 92 used this path. | Implemented correctly | `ansible/roles/ssn_slurm_config/tasks/main.yml`, `ssn/users.py`, `bin/ssn-archive-runner`, live Quadro archive jobs | Harden interrupted-job repair and real backup-hook retries. |
| Archive root required and Slurm unavailable blocks transition. | Archive root is required and `/data/_archive` was used live. Archive execution now depends on Slurm job submission and monitoring; an unavailable Slurm path fails the apply rather than silently doing direct root archive work. A dedicated planner-time Slurm health preflight is still basic. | Implemented partially | `ssn/users.py`, live Quadro archive path and jobs | Add explicit Slurm health gate to inactive transition dry-run. |
| Prune allowlist, symlink safety, report-only build trees. | Implemented for fixture and fake non-fixture manifest/apply paths: fixed allowlisted paths, marker-detected venv/conda envs, symlink remove-link-only handling, and report-only build trees. Live manifests covered fake lifecycle users. | Implemented partially | `ssn/users.py`, `tests/test_users.py`, live Quadro inactive plans | Broaden tests and keep real-user production apply gated until policy signoff. |
| Dry plan writes plan id/hash and real apply requires token. | Implemented for fixture and fake non-fixture inactive lifecycle. Dry-run writes a protected plan with `operation_hash` bound to the desired lifecycle and observed hook set. Apply accepts either `inactive_archive_apply` for the backup-hooked path or `inactive_local_only_archive` for reviewed local-only override. Live tests covered missing allowlist before token consumption, no-hook pre-consumption refusal, hook-backed apply, token reuse failure, and wrong-plan token failure. | Implemented correctly | `ssn/cli.py`, `ssn/users.py`, `ssn-plan-token`, live Quadro inactive plan/token tests | Keep real-user production inactive rollout gated until backup policy approval. |
| Backup hooks after local archive. | Implemented for fixture and fake non-fixture lifecycle with hook directory `/etc/slurm-single-node/archive-hooks.d`, executable hook discovery, environment variables, timeout, stdout/stderr capture, `backup_failed`/`backup_complete` state, retry by rerunning with a new reviewed plan/token, and `ssn-archive-status` reporting. Live Quadro used a dummy dispatch hook for `ssn-lifecycle-hookfail` and `ssn-lifecycle-hooksuccess`, then left it disabled. | Implemented partially | `ssn/users.py`, `ssn/cli.py`, `policies/storage.yml`, live Quadro failing/success hook tests | Integrate real external backup/replication hooks and production approvals. |
| UID/GID tombstones and reactivation semantics. | Fixture and fake non-fixture account removal records original UID/GID and tombstone state. Reactivation without explicit original IDs failed; reactivation with original UID/GID recreated `ssn-lifecycle-restore` and restored `/data` ownership. | Implemented partially | `ssn/users.py`, tests, live Quadro reactivation | Add permanent production tombstone reservation and admin migration controls. |
...
| `users-state.yml` state shape with archive states. | State validation accepts archive fields plus backup status metadata and Slurm archive job metadata. Inactive lifecycle records archive path, operation hash, local-only flag, backup required/status/hook/rc/output, archive job id/state, archive service account/QoS, runner payload/result paths, original UID/GID, `backup_failed`, `backup_complete`, `removal_ready`, and `tombstoned`. Production tombstone migration controls remain incomplete. | Implemented partially | `ssn/users.py`, tests, live Quadro `ssn-test-inactive-prod` and `ssn-lifecycle-*` state | Add production tombstone migration controls and broader restore/rollback workflows. |
...
| `policies/storage.yml` shape. | Policy exists; directory creation, per-job scratch, scratch health marker gating, token-gated quota enablement, all-managed quota reporting, fixture quota apply, fixture and exact-user scratch cleanup apply, production-root retention cleanup, archive hook directory, archive service account/QoS, Slurm-backed archive runner, backup-hook lifecycle, and nondurable storage acknowledgment gating are active for test/fake-user paths. Real-user quota apply, scheduled cleanup, and real backup integration remain incomplete. | Implemented partially | `policies/storage.yml`, `ssn/storage.py`, `ssn/users.py`, storage role, live Quadro quota/lifecycle/cleanup tests | Implement real-user quota apply and scheduled cleanup only after policy signoff. |
...
| Scratch-unhealthy, archive, hook, tombstone tests. | Scratch happy path, per-job cleanup, and unhealthy marker/job rejection were live-tested. Fixture and fake non-fixture inactive dry-plan, local-only archive, Slurm archive job execution, hook failure, hook success, tombstone, token reuse, wrong-plan token, missing allowlist, job cancellation, and UID/GID reactivation were live-tested. Real external backup hooks remain untested. | Implemented partially | `ssn/storage.py`, `ssn/users.py`, live Quadro scratch and inactive lifecycle tests | Add real backup-hook success/failure tests before production inactive rollout. |
| Static tests. | Unit tests, compileall, shell syntax, render checks, `git diff --check`, and remote static tests pass. Tests now include schema validation, capability gates, cli-filter capability checks, drain wait timeout behavior, user-sync idempotence, quota report/enable/fixture-apply safety, tombstone-aware UID/GID allocation, scratch health, cleanup operation hashes, fixture cleanup safety, retention test-artifact cleanup safety, inactive plan reports, backup hook gating, prune manifests, exact non-fixture lifecycle allowlists, Slurm archive runner behavior, protected lifecycle user refusal, GPU verification parsing, recovery plan safety, and module/CUDA validate-only detection/rendering. | Implemented correctly | `tests/`, live session notes | Add CI entrypoint when repo is ready. |
...
| Tokenized test-artifact retention cleanup. | Added `ssn-retention-cleanup`: reports old items under a selected root, requires a `retention_delete` plan token for apply, deletes only direct-child candidates named like SSN test artifacts, skips symlinks and production-looking paths, and rejects token reuse/wrong-plan tokens. Live Quadro used `/tmp/ssn-retention-bridge-root`; a later round added production-root cleanup for SSN-owned roots. | `ssn/safety.py`, `ssn/cli.py`, `bin/ssn-retention-cleanup`, live Quadro retention tests | Keep the test-artifact path as the safest cleanup mode for ad hoc roots. |
| Production-root retention cleanup trial. | `ssn-retention-cleanup --allow-production-roots` can now delete reviewed direct-child artifacts under `/var/lib/slurm-single-node/plans`, `/var/backups/slurm-single-node/users`, and `/var/backups/slurm-single-node/fstab` using root-specific name allowlists and a `retention_delete` token. Live Quadro deleted only temporary `ssn-test-storage-retention-*` artifacts from all three roots, skipped a symlink, and rejected token reuse. | `ssn/safety.py`, `ssn/cli.py`, `docs/production-safety-bridge.md`, live Quadro retention cleanup test | Decide whether to schedule this or keep it as manual reviewed cleanup. |
...
| Fake non-fixture lifecycle allowlist. | Added exact `--allow-lifecycle-user USER` guardrails for non-`ssn-test-*` inactive apply and explicit refusal of protected/admin users such as `root`, `adhil`, and `roshan`. Live Quadro exercised only fake `ssn-lifecycle-*` users and verified missing allowlist fails before token consumption. | `ssn/users.py`, `ssn/cli.py`, tests, live Quadro lifecycle tests | Keep real users gated until production approval. |
| Slurm-backed archive runner. | Added internal `ssn-archive-runner`, Slurm job submission/monitoring through archive account/QoS, runner payload/result artifacts, and archive job metadata in state/status. Live Quadro archive jobs 85, 86, 87, 88, and 92 used this path. | `bin/ssn-archive-runner`, `ssn/users.py`, `ssn/cli.py`, live Quadro lifecycle tests | Harden interrupted archive repair and retry guidance. |
| Microsecond user/state backups. | Backup filenames now include microseconds so repeated lifecycle writes in the same second do not collide. | `ssn/users.py`, live Quadro lifecycle apply repair | Keep retention reporting; add approved pruning only if desired. |
| All-managed quota dry-run. | `ssn-sync-users --quota-report --quota-scope all_managed` reports every active managed user, target home/data/scratch limits, current quota evidence where available, drift, and whether apply is allowed. Live Quadro reported nine active managed users, omitted suspended fake users, and allowed zero applies. | `ssn/storage.py`, `ssn/cli.py`, tests, live Quadro all-managed quota report | Build reviewed real-user quota apply only after production quota values are approved. |
| Exact-user scratch cleanup. | `ssn-scratch-cleanup --allow-cleanup-user USER` reports and applies only aged cache/tmp children for exact users after a `scratch_cleanup` token. Live Quadro deleted only fake-user cache/tmp candidates and rejected wrong-plan and reused tokens. | `ssn/storage.py`, `ssn/cli.py`, `docs/storage-quotas.md`, live Quadro scratch cleanup test | Decide whether to schedule production cleanup or keep exact-user manual cleanup. |
| Protected users.yml names. | `users.yml` validation now refuses protected local users such as `root`, `adhil`, and `roshan`, preventing user/storage workflows from adopting them accidentally. | `ssn/users.py`, tests, live Quadro real-user unchanged checks | Replace the hardcoded protection set with site-configurable protected users if needed. |
| Nondurable storage acknowledgment gate. | Install/apply fail when a nondurable-storage policy requires acknowledgment and the profile lacks `operations.storage_acknowledgements.nondurable_data: true`. The disposable Quadro profile sets the acknowledgment explicitly. | `ssn/ops.py`, `ssn/schema.py`, `profiles/gpu-bisect-quadro-p620.yml`, live Quadro drained install | Add site review docs for production profiles. |
...
- Extend the reviewed-token and exact-allowlist lifecycle pattern from fake
  non-fixture users to real managed users only after external backup policy
  approval.
...
- Decide whether tokenized retention cleanup should remain manual-reviewed or
  become scheduled for SSN-owned roots.
...
- Decide production quota values and add reviewed real-user quota apply beyond
  the current all-managed report-only view.
- Decide whether exact-user scratch cleanup should become scheduled production
  cleanup or remain manual-reviewed.
...
- Replace dummy lifecycle backup hooks with real site backup/replication hooks
  and live-test success/failure behavior.
- Broaden fake non-fixture inactive apply into a production-reviewed real-user
  workflow only after real backup hooks and policy approvals are ready.
- Harden Slurm archive job interruption/resume/repair handling and retry
  guidance for failed archive jobs.
- Add permanent tombstone migration controls and broader restore/rollback
  documentation.
