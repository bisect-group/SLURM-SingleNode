...
failed drained apply had left it drained.
...
ordering health gates, real external backup hook integration beyond dummy
hooks, production inactive rollout for real managed users, permanent tombstone
migration controls, managed CUDA/module update workflows, CUDA toolkit
live-module testing on a host with a toolkit installed, and production-grade
user docs.
...
| `policies/storage.yml` shape. | Policy exists; directory creation, per-job scratch, scratch health marker gating, token-gated quota enablement, fixture quota apply, fixture-only cleanup apply, archive hook directory, archive service account/QoS, Slurm-backed archive runner, and backup-hook lifecycle are active for fixtures and fake non-fixture users. Real-user quota rollout and production cleanup deletion remain incomplete. | Implemented partially | `policies/storage.yml`, `ssn/storage.py`, `ssn/users.py`, storage role, live Quadro quota/lifecycle tests | Implement real-user quota rollout/cleanup enforcement after policy signoff. |
